...
int kThreadState(void) {
  struct thread *t;
  // acquire(&ptable.lock);
  for (t = thread->parent->threads; t < &thread->parent->threads[NTHREAD]; t++) {
    if (t->state == TUNUSED || t->state == TEMBRYO || t->state == TSLEEPING
        || t->state == TRUNNABLE || t->state == TINVALID || t->state == TRUNNING) {
      if (t->tid != thread->tid) {
        return 0;
      }
    }
